		...
		const bool texture_shadow_alpha_bilinear)
{
  ...
  // Otherwise fall back to the original per-Gaussian scalar layout [P].
	  const int tex_res_shadow = static_cast<int>(
		  ...
		  const long shadow_buf_size = (texture_shadow_output_uv && texture_dims.numel() != 0)
		      ? static_cast<long>(texture_alpha.numel())
		      : ((texture_shadow_output_uv && tex_res_shadow > 0) ? static_cast<long>(P) * tex_res_shadow * tex_res_shadow : static_cast<long>(P));
