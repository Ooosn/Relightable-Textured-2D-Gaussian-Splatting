                                     ...
                                     float low_pass_filter_radius)
{
	...
	glm::mat3 cov_pix = glm::transpose(A) * glm::transpose(Vrk) * A;


	cov_pix[0][0] += low_pass_filter_radius;
	cov_pix[1][1] += low_pass_filter_radius;
	...
	)
{
	...
	// 非 hgs 渲染
		...
		if (cov3D_precomp != nullptr)	
		{
			cov.x = cov3D_precomp[0 + idx * 3];
			cov.y = cov3D_precomp[1 + idx * 3];
			cov.z = cov3D_precomp[2 + idx * 3];
		}
		else
		{
	...
	// float2 point_image = {0.5f * (p_proj.x + 1.0f) * W, 0.5f * (p_proj.y + 1.0f) * H};

	float2 point_image = { ndc2Pix(p_proj.x, W), ndc2Pix(p_proj.y, H) };
	uint2 rect_min = {0, 0}, rect_max = {0, 0};
	...
	)
{
	...
	float scene_offset_coefficient = features[P];
	float vehicle_offset_coefficient = features[P+1];
	...
	__shared__ float collected_object_id[BLOCK_SIZE];
	...
	__shared__ float collected_object_id_pointT[BLOCK_SIZE];
	...
	float test_T2 = 0.0f;
	float alpha=0.0f;
	float scaled_offset = 0.0f;
	int object_id_pointT = -1;
	bool noise = false;
	...
	if (range.x + progress_pointT < range.y)
	{
		...
		collected_object_id_pointT[block.thread_rank()] = features[coll_id]; // 写入 高斯对应的 对象ID
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			collected_object_id[block.thread_rank()] = features[coll_id];
		...
		{	
			...
			int object_id_j = (int)(collected_object_id[j]+0.5);
			...
			if(object_id_j == 1 && object_id_pointT == 1)
				scaled_offset = offset * scene_offset_coefficient;
			else if(object_id_j == object_id_pointT)
				scaled_offset = offset * vehicle_offset_coefficient;
			else if(object_id_j >= 100)
				scaled_offset = 0.0f;
			else 
			{	int object_id_j_0 = (int)(collected_object_id[j-3]+0.5);
				int object_id_j_1 = (int)(collected_object_id[j-1]+0.5);
				int object_id_j_2 = (int)(collected_object_id[j+1]+0.5);
				int object_id_j_3 = (int)(collected_object_id[j+3]+0.5);
				int object_id_j_4 = (int)(collected_object_id[j+5]+0.5);
				int object_id_j_5 = (int)(collected_object_id[j+10]+0.5);
				if (object_id_j == 1 &&
					(  object_id_j_0 != 1 || object_id_j_1 != 1 
					|| object_id_j_2 != 1 || object_id_j_3 != 1
					|| object_id_j_4 != 1 || object_id_j_5 != 1))
					scaled_offset = offset * vehicle_offset_coefficient;
				else if (object_id_j != 1 &&
					(  object_id_j_0 == 1 || object_id_j_1 == 1 
					|| object_id_j_2 == 1 || object_id_j_3 == 1
					|| object_id_j_4 == 1 || object_id_j_5 == 1))
					scaled_offset = offset * scene_offset_coefficient;
			...
			if(depth_diff>scaled_offset)	// depthT 刚开始为 0，所以第一个高斯点一定会进入循环
			{	
				while(1)
				{	
					...
					if (isnan(con_o.x)) {
						printf("NaN detected: con_o.x = %f\n", con_o.x);
					}
					if (isnan(con_o.y)) {
						printf("NaN detected: con_o.y = %f\n", con_o.y);
					}
					if (isnan(con_o.z)) {
						printf("NaN detected: con_o.z = %f\n", con_o.z);
					}
					if (isnan(con_o.w)) {
						printf("NaN detected: con_o.w = %f\n", con_o.w);
					}	
					...
					object_id_pointT = (int)(collected_object_id_pointT[pointT]+0.5);
					pointT=pointT+1;
					...
					if(pointT>=BLOCK_SIZE)
					{
						...
						if(i_pointT<rounds)	// 未超出当前块的渲染高斯点数量
						{
							...
							{	
								...
								collected_object_id_pointT[block.thread_rank()] = features[coll_id];
							}
					...
					*/ 
					...
					{	
						...
						{   
							// T = (object_id_j == 3 && object_id_pointT == 3) ? 1.0f : T;
