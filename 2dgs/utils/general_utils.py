...
def colormap(img, cmap='jet'):
    """
    将二维数组通过 matplotlib 上色后转成 torch 张量。
    这里强制使用 Agg 后端自己创建 canvas，避免在 TkAgg 等交互式后端上
    缺少 `tostring_rgb` 方法导致的 AttributeError。
    """
    ...
    # 使用 Figure + FigureCanvasAgg 而不是当前全局 backend 的 canvas
    ...
    # 在 Agg canvas 上渲染并取 RGB 缓冲区
    canvas.draw()
    data = np.frombuffer(canvas.buffer_rgba(), dtype=np.uint8)
    data = data.reshape(canvas.get_width_height()[::-1] + (4,))  # RGBA
    data = data[..., :3]  # 丢掉 alpha

    img_torch = torch.from_numpy(data / 255.).float().permute(2, 0, 1)
    plt.close(fig)
    return img_torch